...
#include <ESP32-HUB75-MatrixPanel-I2S-DMA.h>    //library untuk Panel Dot
...
//definisi pin untuk HUB08/75
#define R1_PIN 4    //pin Red 1
#define G1_PIN 13   //pin Green 1
#define B1_PIN 13   //pin Blue 1
#define R2_PIN 15   //pin Red 2
#define G2_PIN 13   //pin Green 2
...
#define PANEL_CHAIN 2   //chaining default(horizontal)

//definisi font
#ifndef NO_GFX
#include <Fonts\ARLRDBD7pt7b.h>
#include <Fonts\ARLRDBD8pt7b.h>
...
uint16_t textColor = dma_display->color565(255, 0, 0);

//variabel global untuk autocenter
int max_x = 128;              //Nilai maksimum sumbu x yang mau dibatasi
int max_y = 32;               //Nilai maksimum sumbu y yang mau dibatasi
...
void setup() {
  ...
  file = SD.open("/DATABASE_JR205.txt");      //open database.txt
  if (!file){
...
void autoCenter() {
  if (i == 0) { //Hapus tampilan saat nilai dari ads berganti
    ...
    dma_display->setTextColor(textColor);   //Set warna huruf
    ...
    if (Z.length() > 0 && ZA.length() == 0 ){ //Jika hanya Z dan tidak ada ZA
      if (spaceIndexZ != -1) { //Jika ada spasi
        if (Z == "UJI COBA"){ //Khusus Kalimat UJI COBA
          dma_display->setFont(&ARLRDBD14pt7b); //Set Font
          dma_display->setTextSize(1);          //Set pengali font
          ...
          //dma_display->getTextBounds() mengembalikan posisi dan ukuran teks dalam variabel yang diberikan
          dma_display->getTextBounds(Z.c_str(), 0, 0, &x1_5, &y1_5, &textWidth5, &textHeight5);
          ...
          dma_display->setCursor(xCenter5, yCenter5);
          dma_display->print(Z);
          ...
          if (Z_1.length() <= 6) {
            dma_display->setFont(&ARLRDBD10pt7b);   //Set Font
            dma_display->setTextSize(1);            //Set pengali font
          ...
          //Menghitung bounding box (ukuran dan posisi) teks pada posisi (0, 0)
          //dma_display->getTextBounds() mengembalikan posisi dan ukuran teks dalam variabel yang diberikan
          dma_display->getTextBounds(Z_1.c_str(), 0, 0, &x1_1, &y1_1, &textWidth1, &textHeight1);
          ...
          dma_display->setCursor(xCenter1, (16 - textHeight1) / 2 - y1_1);
          dma_display->print(Z_1);
          ...
          //Menghitung bounding box (ukuran dan posisi) teks pada posisi (0, 0)
          //dma_display->getTextBounds() mengembalikan posisi dan ukuran teks dalam variabel yang diberikan
          dma_display->getTextBounds(Z_2.c_str(), 0, 0, &x1_2, &y1_2, &textWidth2, &textHeight2);
          ...
          dma_display->setCursor(xCenter2, 17 + (16 - textHeight2) / 2 - y1_2);
          dma_display->print(Z_2);
          i = 3; // Set flag agar hanya tampil sekali
        }
      }
        ...
        if (Z.length() <= 4) {
          dma_display->setFont(&ARLRDBD19pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else if (Z.length() == 5) {
          dma_display->setFont(&ARLRDBD17pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else if (Z.length() == 6) {
          dma_display->setFont(&ARLRDBD15pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else if (Z.length() == 7) {
          dma_display->setFont(&ARLRDBD13pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else if (Z.length() == 8) {
          dma_display->setFont(&ARLRDBD12pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else if (Z.length() == 9) {
          dma_display->setFont(&ARLRDBD10pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        } else {
          dma_display->setFont(&ARLRDBD9pt7b);   //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        }
        ...
        //Menghitung bounding box (ukuran dan posisi) teks pada posisi (0, 0)
          //dma_display->getTextBounds() mengembalikan posisi dan ukuran teks dalam variabel yang diberikan
        dma_display->getTextBounds(Z.c_str(), 0, 0, &x1, &y1, &textWidth, &textHeight);
        ...
        dma_display->setCursor(xCenter, yCenter);
        dma_display->print(Z.c_str());
        i = 3;// Set flag agar hanya tampil sekali
      }
    }
    else {//jika ada ZA
        ...
        //Menghitung bounding box (ukuran dan posisi) teks pada posisi (0, 0)
          //dma_display->getTextBounds() mengembalikan posisi dan ukuran teks dalam variabel yang diberikan
        dma_display->getTextBounds(Z.c_str(), 0, 0, &x1_3, &y1_3, &textWidth3, &textHeight3);
        ...
        dma_display->setCursor(xCenter3, (16 - textHeight3) / 2 - y1_3); //karena y data Z akan ditampilkan dari senter bagian display atas
        dma_display->print(Z);
        ...
        if (ZA.length() == 9) {//PENUMPANG
          ...
          dma_display->setFont(&ARLRDBD7pt7b);    //Set Font
          dma_display->setTextSize(1);            //Set pengali font
        }
        ...
        dma_display->getTextBounds(ZA.c_str(), 0, 0, &x1_4, &y1_4, &textWidth4, &textHeight4);
        ...
        dma_display->setCursor(xCenter4, 17 + (16 - textHeight4) / 2 - y1_4); //karena y data Z akan ditampilkan dari senter bagian display atas
        dma_display->print(ZA);
        i = 3; // Set flag agar hanya tampil sekali
    }
  }
  else {
